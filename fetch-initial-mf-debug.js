#!/usr/bin/env node
/**
 * fetch-initial-mf.js — Fetch initial 60 jours via DPClim infrahoraire-6m
 * Usage : MF_CLIM_API_KEY=ta_clé node fetch-initial-mf.js
 * Max 31 jours par commande → 2 tranches de 30 jours
 */
...
async function commandeEtRecup(fromDate, toDate) {
  ...
  const cmdPath = `/public/DPClim/v1/commande-station/infrahoraire-6m` +
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (r.status === 201) {
      require("fs").writeFileSync("/tmp/mf-sample.csv", r.body.slice(0, 5000)); console.log(`\n  ✅ Fichier prêt (${Math.round(r.body.length/1024)} Ko)`);
      return r.body;
    }
...
async function main() {
  ...
  console.log(`▶ Fetch initial DPClim infrahoraire-6m — Station ${STATION_ID}`);
